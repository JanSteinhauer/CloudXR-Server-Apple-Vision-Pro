...
// Include the C header WITHOUT the DLL import/export macros
...
void print_help()
{
    cout << "Available commands:\n"
         << "  - StartCxrService <version> - Start the CloudXR service with the specified version\n"
         ...
         << "  - GetCryptoKeyFingerprint <algorithm> - Get the cryptographic key fingerprint (algorithms: 0=MD5, 1=SHA-1, 2=SHA-256, 3=SHA-512)\n"
         << "  - SetClientId <client_id> - Set the client ID and get the token\n"
         << "  - GetCxrServiceStatus    - Get the current service connection status\n"
...
void handle_start_cxr_service_command(nv_rpc_client_t client, const string &parameter)
{
...
void handle_stop_cxr_service_command(nv_rpc_client_t client)
{
...
void handle_get_supported_versions_command(nv_rpc_client_t client)
{
...
void handle_get_cxr_service_json_path_command(nv_rpc_client_t client)
{
...
void handle_get_crypto_key_fingerprint_command(nv_rpc_client_t client, const string &parameter)
{
    if (parameter.empty())
    {
        cerr << "Usage: GetCryptoKeyFingerprint <algorithm>\n";
        return;
    }
    ...
    if (algorithm < 0 || algorithm > 3)
    {
        ...
        cerr << "Valid algorithms: 0=MD5, 1=SHA-1, 2=SHA-256, 3=SHA-512\n";
        return;
    }
    ...
    nv_rpc_result_t result = g_rpc_functions.get_crypto_key_fingerprint(client, (nv_crypto_algorithm_t)algorithm, &fingerprint[0], fingerprint.size() - 1);
...
void handle_set_client_id_command(nv_rpc_client_t client, const string &parameter)
{
    if (parameter.empty())
    {
        cerr << "Usage: SetClientId <client_id>\n";
        return;
    }
    ...
    size_t token_size = 0;
    nv_rpc_result_t result = g_rpc_functions.set_client_id(client, parameter.c_str(), parameter.length(),
...
void handle_get_cxr_service_status_command(nv_rpc_client_t client)
{
    ...
    cout << "  OpenXR Log File Path: " << (status.openxr_log_file_path_length > 0 ? std::string(status.openxr_log_file_path, status.openxr_log_file_path_length) : "[not available]") << "\n";
}

void handle_shutdown_manager_command(nv_rpc_client_t client)
{
...
int try_load_dll_from_fallback_paths()
{
    const std::array<const char *, 3> dll_paths = {
...
int load_rpc_client_dll()
{
    ...
    g_rpc_functions.disconnect = (pfn_nv_rpc_client_disconnect)GetProcAddress(g_dll_handle, "nv_rpc_client_disconnect");
    g_rpc_functions.start_cxr_service = (pfn_nv_rpc_client_start_cxr_service)GetProcAddress(g_dll_handle, "nv_rpc_client_start_cxr_service");
    g_rpc_functions.stop_cxr_service = (pfn_nv_rpc_client_stop_cxr_service)GetProcAddress(g_dll_handle, "nv_rpc_client_stop_cxr_service");
    g_rpc_functions.get_supported_versions = (pfn_nv_rpc_client_get_supported_versions)GetProcAddress(g_dll_handle, "nv_rpc_client_get_supported_versions");
    g_rpc_functions.get_cxr_service_json_path = (pfn_nv_rpc_client_get_cxr_service_json_path)GetProcAddress(g_dll_handle, "nv_rpc_client_get_cxr_service_json_path");
    g_rpc_functions.get_crypto_key_fingerprint = (pfn_nv_rpc_client_get_crypto_key_fingerprint)GetProcAddress(g_dll_handle, "nv_rpc_client_get_crypto_key_fingerprint");
    g_rpc_functions.free_string_array = (pfn_nv_rpc_client_free_string_array)GetProcAddress(g_dll_handle, "nv_rpc_client_free_string_array");
    g_rpc_functions.get_error_string = (pfn_nv_rpc_client_get_error_string)GetProcAddress(g_dll_handle, "nv_rpc_client_get_error_string");
    g_rpc_functions.set_client_id = (pfn_nv_rpc_client_set_client_id)GetProcAddress(g_dll_handle, "nv_rpc_client_set_client_id");
    g_rpc_functions.get_cxr_service_status = (pfn_nv_rpc_client_get_cxr_service_status)GetProcAddress(g_dll_handle, "nv_rpc_client_get_cxr_service_status");
    g_rpc_functions.shutdown_manager = (pfn_nv_rpc_client_shutdown_manager)GetProcAddress(g_dll_handle, "nv_rpc_client_shutdown_manager");
...
int start_stream_manager_subprocess(const char *stream_manager_path)
{
    ...
    if (!SetInformationJobObject(g_stream_manager_job, JobObjectExtendedLimitInformation, 
    ...
    snprintf(log_filename, sizeof(log_filename),
             "NvStreamManager_%04d%02d%02d_%02d%02d%02d.log",
...
void cleanup_stream_manager_subprocess()
{
...
int initialize_rpc_client(nv_rpc_client_t *client)
{
...
void run_interactive_session(nv_rpc_client_t client)
{
